...
var BinMap = React.createClass({
  processData: function () {
    ...
    var selectedBin = this.props.pickFrontSelectedBin
      ? this.props.pickFrontSelectedBin
      : false
    ...
    var leftCol = [],
      ...
      maxLeftCount = 0,
      maxRightCount = 0,
      maxCenterLeftCount = 0,
      maxCenterRightCount = 0,
      maxBlockHeight = 0,
      style = null,
      maxWidth = null
    if (
      ...
      for (var k in bindata) {
        if (bindata[k].direction === allresourceConstants.BIN_GROUP_LEFT) {
          ...
          bindata[k].direction === allresourceConstants.BIN_CENTER_LEFT
        ) {
          maxCenterLeftCount++
        } else if (
          bindata[k].direction === allresourceConstants.BIN_CENTER_RIGHT
        ) {
          maxCenterRightCount++
        } else if (
          bindata[k].direction === allresourceConstants.BIN_GROUP_RIGHT
        ) {
          maxRightCount++
        }
      }
    ...
    maxCenterRightCount === 0 &&
    maxCenterLeftCount === 0 &&
    (maxRightCount > 1 || maxLeftCount > 1)
      ? (style = {
    ...
    if (
      ...
      for (let i = 0; i < bindata.length; i++) {
        if (
          (bindata[i].direction === allresourceConstants.BIN_GROUP_LEFT &&
            (bindata[i].ppsbin_id === selectedBin ||
              bindata[i].selected_state === true)) ||
          (bindata[i].direction === allresourceConstants.BIN_CENTER_LEFT &&
            (bindata[i].ppsbin_id === selectedBin ||
              bindata[i].selected_state === true))
        ) {
          leftCol.push(<li key={k} style={style} className={"sel"}></li>)
        } else if (
          (bindata[i].direction === allresourceConstants.BIN_GROUP_RIGHT &&
            (bindata[i].ppsbin_id === selectedBin ||
              bindata[i].selected_state === true)) ||
          (bindata[i].direction === allresourceConstants.BIN_CENTER_RIGHT &&
            (bindata[i].ppsbin_id === selectedBin ||
              bindata[i].selected_state === true))
        ) {
          rightCol.push(<li key={k} style={style} className={"sel"}></li>)
        } else if (
          (bindata[i].direction === allresourceConstants.BIN_GROUP_CENTER &&
            bindata[i].ppsbin_id === selectedBin) ||
          bindata[i].selected_state === true
        ) {
          centerbar = "blue"
      ...
      for (var k in data) {
        if (data.hasOwnProperty(k)) {
          ...
          if (data[k] === allresourceConstants.BIN_GROUP_LEFT) {
            ...
            rightCol.push(
              <li key={k} style={style} className={isSelected}></li>
            )
          }
        }
      }
    ...
    if (rightCol.length === 0) {
      rightCol.push(
        <li style={{ height: "105px" }} className={"col3Zero"}></li>
      )
    }
    ...
    return (
      ...
      >
        <div className="mapCont">
          <div
            style={
              mapStructure.centerbar === "blue"
                ? { backgroundColor: "#0390FF" }
                : {}
            }
            className="msuSpace"
          ></div>
