...
var Bins = React.createClass({
  ...
  _createBinLayouts: function(
    aBins,
    lastHBin,
    lastVBin,
    seatType,
    screenId,
    binCoordinatePlotting,
    dataToDisassociateTote
  ) {
    if (
      (aBins.constructor !== Array && aBins.length < 1) ||
      !lastHBin.length ||
      !lastVBin.length
    ) {
      //no bins found
    ...
    for (var i = 0; i < aBins.length; i++) {
      ...
      if (binCoordinatePlotting == true || binCoordinatePlotting == 'true') {
        aHTMLBins.push(
          ...
          >
            <Bin
              dataToDisassociateTote={dataToDisassociateTote}
              binData={aBins[i]}
              screenId={screenId}
  ...
  render: function() {
    var aHTMLBins = this._createBinLayouts(
      this.state.aBins,
      this.state.lastHBin,
      this.state.lastVBin,
