...
var Header = React.createClass({
  ...
  enableException: function() {
    if (mainstore.getScreenId() === appConstants.UNIVERSAL_DOCK_UNDOCK) {
      var data1 = {
        event_name: "",
        event_data: {},
        source: "ui"
      ...
      data1["event_data"]["event"] = "reprint_packlist"
  ...
  getExceptionMenu: function() {
    ...
    if (
      ...
      screenId === appConstants.ARA_PICK_FRONT
    )
      this.exceptionMenu = ""
  ...
  getSearchItemMenu: function() {
    if (
      mainstore.orphanSearchAllowed() &&
      mainstore.getScreenId() !== appConstants.ARA_PICK_FRONT
    ) {
      this.searchMenu = (
  ...
  getUsernameMenu: function() {
    if (mainstore.getScreenId() === appConstants.ARA_PICK_FRONT) {
      this.usernameMenu = (
        <div className="actionItem">{this.state.username}</div>
      )
    }
  },
  ...
  render: function() {
    ...
    var araPickFrontFlow =
      mainstore.getScreenId() === appConstants.ARA_PICK_FRONT ? true : false
    var invoiceFlow =
    ...
    const isFrontScreen = this.state.frontScreen === appConstants.FRONT
      ...
      <div>
        ...
        <div className="actionMenu" id="actionMenu">
          ...
          <div
            ...
            onClick={
              araPickFrontFlow
                ? this.showModal.bind(this, null, appConstants.CONFIRM_LOGOUT)
                : this.logoutSession
            }
          >
            {_("Logout")}
