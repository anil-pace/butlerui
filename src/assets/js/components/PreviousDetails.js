...
var PreviousDetails = React.createClass({
  ...
  render: function () {
    ...
    let header
    header =
      type === "pick" ? _("Previous Pick Details") : _("Previous Put Details")
    let typeToShow = this.props.typeToShow || header
    ...
    return (
      <div
        className={
          customizeClass ? "p-put-details " + customizeClass : "p-put-details"
        }
      >
        <div className="p-put-head">{typeToShow.toUpperCase()}</div>
          ...
          {Object.keys(previousDetails).map(function (key, idx) {
            if (idx === 0) {
              ...
              if (str) {
                var txtToDisplay = []
                var frmStart = (
                  <span className="p-put-value">
                    {str.substring(0, str.length - 3)}
                  </span>
                )
                var frmLast = (
                  <span className="p-put-value-extra">
        ...
        {!compSpecificPreviousLocation && showPrevLocation === true ? (
          <div>
            ...
            <div className="p-put-content">
              {Object.keys(previousDetails).map(function (key, idx) {
                if (idx === 2) {
                  // comparing with idx & not key as key is client configurable
                  var str = previousDetails[key].trim()
                  return (
                    <section key={key + idx} className="p-put-row">
                      {str ? (
